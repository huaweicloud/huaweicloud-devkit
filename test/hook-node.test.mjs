...
test('node hook denies null payload (fail-closed #689)', () => {
...
test('node hook denies non-object JSON payloads (fail-closed #689)', () => {
  for (const raw of ['123', '"foo"', '["x"]']) {
    const result = runHookRaw(raw);

    assert.equal(result.status, 0, `payload ${raw} should not crash: ${result.stderr}`);
    const output = JSON.parse(result.stdout);
...
test('node hook denies empty command text (fail-closed #689)', () => {
...
test('node hook denies whitespace-only command text (fail-closed #689)', () => {
  const result = runHook({ tool_name: 'Bash', tool_input: { command: '   ' } });
